...
def main() -> None:
    ...
    predictor = None if args.without_model else ThermalLoadPredictor(ROOT / "models")
    table, _ = run_strategy_comparison(cfg, scenario_names(), args.duration, predictor)
    output = ROOT / "results" / "tables" / "strategy_comparison.csv"
    output.parent.mkdir(parents=True, exist_ok=True)
    table.to_csv(output, index=False)
    plot_strategy_comparison(table, ROOT / "results" / "figures" / "strategy_comparison.png")
